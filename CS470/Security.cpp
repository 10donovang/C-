...
/******************************************
 * Place to call functions/declare variables
 *****************************************/
enum Control{
  none = 0, read = 1, exec = 4 
};

bool securityConditionRead(Control subjectControl)
{
return (subjectControl >= 1);
}

bool securityConditionWrite(Control subjectControl)
{
return subjectControl == 4;
}
...
public:
  ...
  ~StudentGrade();
   ...
   void   displayScores(bool, bool, string); // display scores on screen
   void   editScores(bool, bool, string); // interactively edit score
 ...
 *******************************************/
void StudentGrade::editScores(bool read, bool exec, string logName)
{
   ...
   // display score list
   cout << "Score list\n";
   if((read == true &&  logName == name) || exec == true){
       cout << "\tScore \tWeight\n";
   }
   for (int iScore = 0; iScore < scores.size(); iScore++)
   {
 ...
 * Display scores
 ***********************************************/
void StudentGrade::displayScores(bool read, bool exec, string logName)
{
   if((read == true &&  logName == name) || exec == true){
      if (scores.size() == 0)
         return;
...
public:
   ...
   void display(bool, bool, string);
   void interact(bool, bool, string);
private:
 ...
 ***********************************************/
void Interface::interact(bool read, bool exec, string logName) 
{
   int iSelected;
   while (-1 != (iSelected = promptForStudent()))
   {
      // edit grades as necessary
      students[iSelected].editScores(read, exec, logName);
         
      // show the results
      
      students[iSelected].displayScores(read, exec, logName);
 ...
 * Display stuff
 *************************************************/
void Interface::display(bool read, bool exec, string fullName)
{
   for (int i = 0; i < students.size(); i++)
   
      students[i].displayScores(read, exec, fullName);
}
...
const User users[] =
{
   { "Bob",  "passwordBob", exec, "Bob"},
   { "Hans", "passwordHans", exec, "Hans"},
   { "Sam",  "passwordSam", read, "Samual Stevenson"},
   { "Sue",  "passwordSue", read, "Susan Bakersfield"},
   { "Sly",  "passwordSly", read, "Sylvester Stallone"},
   {"hacker", "hacker", none, "hacker"}
};
...
int main()
{
   int idUser = authenticate();
   
   User person = users[idUser];
   
   bool read = securityConditionRead(person.person);
   bool exec = securityConditionWrite(person.person);
   string fullName = person.fullname;
   

   Interface interface;

   interface.interact(read, exec, fullName);
